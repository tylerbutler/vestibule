...
pub fn wrong_jwks() -> String {
  unknown_jwks
}

pub fn mixed_jwks() -> String {
  string.drop_end(public_jwks, 2)
  <> ",{\"kty\":\"EC\",\"kid\":\"ec-key\",\"use\":\"sig\",\"alg\":\"ES256\",\"crv\":\"P-256\",\"x\":\"AA\",\"y\":\"AA\"}]}"
}
...
fn sign_bits(message: BitArray, key: SignKey) -> BitArray {
  sign_key.match(
    key,
    fn(_, _, _, _, _) { <<>> },
    fn(_, _, _, _, _, _) { sign(message, key) },
    fn(_, _, _, _, _, _, _, _, _, _, _, _) { sign(message, key) },
    fn(_, _, _) { <<>> },
  )
}

@external(erlang, "vestibule_oidc_test_ffi", "sign")
