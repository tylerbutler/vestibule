...
pub fn jwks_try_init_named_returns_error_for_duplicate_table_test() {
  ...
  let assert Error(_) = jwks.try_init_named(name)
  Nil
}
...
pub fn parse_token_response_success_test() {
  ...
  assert strategy.exchange_credentials(exchange)
    == credentials.new(
...
pub fn parse_token_response_without_refresh_token_test() {
  ...
  assert credentials.token(creds) == "test_token"
  assert credentials.refresh_token(creds) == None
  let assert Ok(id_token) =
    dict.get(strategy.exchange_artifacts(exchange), "id_token")
  assert decode.run(id_token, decode.string) == Ok("h.p.s")
}
...
pub fn parse_token_response_error_test() {
  ...
  let assert Error(_) = vestibule_apple.parse_token_response(body)
  Nil
}
