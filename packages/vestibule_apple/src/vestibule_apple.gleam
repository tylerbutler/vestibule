...
/// ## Setup
///
/// Call `vestibule_apple.init()` once at application startup to initialize the
/// internal ID token cache, then use `vestibule_apple.strategy()` to create
/// the strategy.
///
/// ```gleam
/// vestibule_apple.init()
/// let strategy = vestibule_apple.strategy()
/// ```
...
/// Initialize the Apple strategy's internal ID token cache.
///
/// Must be called once at application startup before handling any
/// authentication flows. Safe to call multiple times.
pub fn init() -> Nil {
  id_token_cache.init()
}
...
///
/// Apple requires `response_mode=form_post` for the authorization URL,
/// which is added automatically. The client_secret must be a signed JWT
...
/// Decode an Apple ID token JWT and extract user claims.
///
/// Splits the JWT by ".", takes the payload segment (index 1),
/// base64url-decodes it, and parses the JSON claims.
///
/// Note: This does NOT verify the JWT signature. For production use,
/// signature verification against Apple's public keys is recommended
...
pub fn decode_id_token(
  ...
  case string.split(jwt, ".") {
    [_, payload, ..] -> {
      case base64_url_decode(payload) {
        Ok(json_string) -> parse_id_token_claims(json_string)
        Error(Nil) ->
          Error(error.UserInfoFailed(
            reason: "Failed to base64-decode Apple ID token payload",
          ))
      }
    }
    _ ->
      Error(error.UserInfoFailed(
        reason: "Invalid Apple ID token format: expected JWT with 3 segments",
      ))
  }
}

/// Parse the decoded JWT claims JSON into a uid and UserInfo.
///
/// Extracts:
/// - `sub` claim as the uid
/// - `email` claim as the email (only if `email_verified` is "true")
/// - `email` as nickname regardless of verification
///
/// Note: Apple's `email_verified` is a string "true"/"false", not a boolean.
pub fn parse_id_token_claims(
  json_string: String,
) -> Result(#(String, user_info.UserInfo), AuthError(e)) {
  let decoder = {
  ...
  case json.parse(json_string, decoder) {
    ...
    _ ->
      Error(error.UserInfoFailed(
        reason: "Failed to parse Apple ID token claims",
      ))
  }
}
  ...
  case httpc.send(req) {
    Ok(response) -> {
      case parse_token_response(response.body) {
        Ok(#(creds, id_token)) -> {
          // Store the id_token in the ETS cache so fetch_user can access it
          case id_token {
            Some(token) -> id_token_cache.store(creds.token, token)
            None -> Nil
          }
          Ok(creds)
        }
...
fn do_fetch_user(
  ...
  case id_token_cache.retrieve(creds.token) {
    Ok(id_token) -> decode_id_token(id_token)
    Error(Nil) ->
      Error(error.UserInfoFailed(
