...
/// automatically. The `config.ClientSecret(jwt)` value must contain a
...
pub fn verify_id_token(
  ...
  verify_id_token_raw(jwt:, keys:, client_id:)
  |> result.map_error(apple_id_token_error)
}

fn verify_id_token_raw(
  jwt jwt: String,
  keys keys: List(VerifyKey),
  client_id client_id: String,
) -> Result(#(String, user_info.UserInfo), jwt.ParseError) {
  let claims = [
  ...
  use #(subject, email, email_verified) <- result.try(jwt.decode(
    jwt: jwt,
    using: payload_decoder,
    claims: claims,
    keys: keys,
  ))
  let verified_email = case email, email_verified {
    Some(email_address), Some(True) -> Some(email_address)
    Some(_), Some(False) | Some(_), None | None, Some(_) | None, None -> None
  }
  Ok(#(
    subject,
    user_info.new()
...
fn apple_id_token_error(jwt_error: jwt.ParseError) -> AuthError(e) {
  case jwt_error {
    jwt.InvalidSignature | jwt.NoMatchingKey ->
      error.user_info(reason: "Apple ID token signature verification failed")
    jwt.TokenExpired(_) -> error.user_info(reason: "Apple ID token has expired")
    jwt.InvalidIssuer(_, _) ->
      error.user_info(reason: "Apple ID token issuer is invalid")
    jwt.InvalidAudience(_, _) ->
      error.user_info(
        reason: "Apple ID token audience does not match client_id",
      )
    jwt.UnsupportedAlgorithm(_) ->
      error.user_info(
        reason: "Apple ID token must use the RS256 signing algorithm",
      )
    jwt.MalformedToken
    | jwt.InvalidHeaderEncoding
    | jwt.InvalidPayloadEncoding
    | jwt.InvalidSignatureEncoding
    | jwt.InvalidHeaderJson(_)
    | jwt.InvalidPayloadJson(_)
    | jwt.TokenNotYetValid(_)
    | jwt.InvalidSubject(_, _)
    | jwt.InvalidId(_, _)
    | jwt.MissingClaim(_)
    | jwt.ClaimDecodingError(_, _)
    | jwt.InvalidCustomClaim(_)
    | jwt.PayloadDecodingError(_) ->
      error.user_info(reason: "Failed to verify Apple ID token")
  }
}
...
fn do_fetch_user(
  ...
  use #(user_id, user_information) <- result.try(
    case verify_id_token_raw(jwt: id_token, keys: keys, client_id: client_id) {
      ...
      Error(jwt_error) -> Error(apple_id_token_error(jwt_error))
      Ok(user) -> Ok(user)
    },
  )
