...
client assertion mode, pass a signed JWT as `config.ClientSecret(jwt)` in your
...
as `config.ClientSecret(jwt)` when you build the vestibule config.

## Notes

- Apple requires `response_mode=form_post`, which this strategy adds for you.
- The `config.ClientSecret(jwt)` value must be a JWT generated by your app.
