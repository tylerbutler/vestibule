...
Apple does not use a static client secret. Until this strategy supports a dedicated
client assertion mode, pass a signed JWT with `config.client_secret_auth(jwt)` in your
vestibule client config. Generate the JWT from your Apple Developer account
details:
...
with `config.client_secret_auth(jwt)` when you build the vestibule config.

## Notes

- Apple requires `response_mode=form_post`, which this strategy adds for you.
- The `config.client_secret_auth(jwt)` value must be a JWT generated by your app.
