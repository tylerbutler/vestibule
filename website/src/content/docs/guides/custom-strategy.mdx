...
<section id="strategy-type">
  ...
  <p>
    The provider name, default scopes, and builder functions define the full
    contract. The custom error payload type flows through{" "}
  ...
  <div className="step-item">
    ...
    <div>
      ...
      <p>
        ...
        <code>strategy.Strategy(e)</code> when the provider only needs built-in
        error kinds, or <code>strategy.Strategy(YourError)</code> when it wraps
        a custom domain-specific error payload with <code>error.custom</code>.
      </p>
