...
|---|---:|---:|---|
| `vestibule` | `0.1.0` | 13 | Strategy-based OAuth2 authentication for Gleam |
...
| `vestibule` | [`vestibule/authorization_request`](/docs/reference/vestibule-authorization_request) | An opaque value carrying everything the middleware needs to start an authorization flow: the URL to redirect the browser to, the CSRF `state`, and the PKCE `code_verifier` that must be stored for the callback. |
...
| `vestibule` | [`vestibule/oidc`](/docs/reference/vestibule-oidc) | OpenID Connect Discovery support for auto-configuring strategies. |
...
| `vestibule` | [`vestibule/strategy`](/docs/reference/vestibule-strategy) | Provider-strategy interface. A `Strategy(e)` is an opaque record bundling the four functions every OAuth/OIDC provider must implement: build authorize URL, exchange code, refresh token, and fetch user. |
